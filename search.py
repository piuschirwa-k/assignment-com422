...
def uniformCostSearch(problem):
    ...
    uniform = util.PriorityQueue()
    # Make an empty list of explored nodes
    iterated = []
    # Make an empty list of actions
    action = []
    # Place the starting point in the priority queue
    uniform.push((problem.getStartState(), action), problem)
    while uniform:
        node, actions = uniform.pop()
        if not node in iterated:
            iterated.append(node)
            if problem.isGoalState(node):
                return actions
            for successor in problem.getSuccessors(node):
                ...
                uniform.push((coordinate, nextActions), nextCost)
    return []
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    action= []
    # Place the starting point in the priority queue
    aStar.push((problem.getStartState(), action), heuristic(problem.getStartState(), problem))
    while aStar:
